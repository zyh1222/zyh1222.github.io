...
readTextFile("data/data.json", function (text6) {
      parseData(text6)
      for (let i = 0; i < rawdata.length; i++) {
        var d = rawdata[i];
        if ((d["created_int"] > 1466438400000) && (d["created_int"] < 1466452800000)) {
          // console.log(d)
          rawdata1.push(d)
        }
        if ((d["created_int"] > 1466452800000) && (d["created_int"] < 1466467200000)) {
          rawdata2.push(d)
        }
        if ((d["created_int"] > 1466467200000) && (d["created_int"] < 1466481600000)){
          rawdata3.push(d)
        }
        if (d["created_int"] > 1466481600000 && (d["created_int"] < 1466496000000)){
          rawdata4.push(d)
        }
        if ((d["created_int"] > 1466496000000) && (d["created_int"] < 1466510400000)){
          rawdata5.push(d)
        }
        if (d["created_int"] > 1466510400000){
          rawdata6.push(d)
        }
      }
    })
  ...
  function search (searchquery, options, fromPaging) {
    ...
    if (q) {
      ...
      var doneFunc = function (err, data_u) {
        ...
        const data =  {
          'rows':[],
          'fields':['created_at','username','retweets','favorites','text','permalink'],
          'counts':[1]
        }
        ...
        for (let i = 0; i < currRawdata.length; i++) {
          ...
          for (let j = 0; j < searchqueryArray.length; j++) {
            if (!currRawdata[i].text.toLowerCase().includes(searchqueryArray[j])){
              found = false
              break
            }
          }
        ...
        //     for (let j = 0; j < searchqueryArray.length; j++) {
        //       var index = 0
        //       if(currRawdata[i].tweet.toLowerCase()[index,currRawdata[i].tweet.toLowerCase().length-1].includes(searchqueryArray[j])){
        //         index = currRawdata[i].tweet.toLowerCase()[index,currRawdata[i].tweet.toLowerCase().length-1].indexOf(searchqueryArray[j])
        //       }
        //
        //
        //     }
