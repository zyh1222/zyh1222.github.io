...
let brushScaleAll = 50
let brushScaleBrush = 120
let brushScaleClick = 50
...
function draw_context() {
    ...
    if (modesvg === null) {
        modesvg = d3.select("#selecttext").append('svg').attr('id', 'nn').attr("height", 340).attr('position', 'absolute').attr('bottom', '0px').attr('left', '15px')
    }
    let svg = modesvg
    ...
    svg.selectAll("rect_context")
        ...
        .attr("y", function (d, i) {
            // console.log(d)
            if(i<8){
                return graph.height / 5 * i + 10
            }else{
                return graph.height / 5 * (i-8) + 10
            }
    ...
    svg.selectAll("text_context")
        ...
        .attr("y", function (d, i) {
            if(i<8){
                return graph.height / 5 * i + 25

            }else{
                return graph.height / 5 * (i-8) + 25
            }
...
function to_context_tree(keyword, select) {
    ...
    function context_tree(keyword, select) {
        ...
        var timeticks = ['4am', '8am', '12am', '16pm', '20pm', '24pm']
        ...
        var scaler = d3.scalePow()
            .exponent(0.2*6/max_select)
            .range([18*6/max_select, 31*6/max_select])
            .domain([10, 2000]);
        ...
        keywords.append('text')
            ...
            .text(d => d.key)
            .attr("font-size", 26*6/max_select)
            .style('opacity', 0.15)
        ...
        text
            ...
            .attr("y", function (d) {
                ...
                return y - del*6/max_select
            })
            .attr("width", function (d) {
                return scaler(d.freq) * d.name.length / 2
            })
            .attr("height", function (d) {
                return scaler(d.freq) * 1.2*6/max_select
            })
...
function histChart(data = []) {
    ...
    var tooltip = floatingTooltip('gates_tooltip', 240);
    var margin = {top: 10, right: 20, bottom: 50, left: 0},
        width = 1200 - margin.left - margin.right,
        height = 180 - margin.top - margin.bottom;

    const brush = d3.brushX()
        .extent([[margin.left, margin.top], [width, height+10]])
        .on("end", brushended);
...
// append the svg object to the body of the page
    ...
    var x = d3.scaleTime()
        .domain([new Date("2016-06-21 00:00"), new Date("2016-06-22 00:00")])
        .range([0, width]);

    var xb = d3.scaleTime()
        .domain([0, width])
        .range([new Date("2016-06-21 00:00"), new Date("2016-06-22 00:00")]);
    ...
    function brushended() {
        ...
        const time_index =  {'2100': 0, '21020': 1, '21040': 2, '2110': 3, '21120': 36, '21140': 42, '2120': 6, '21220': 66, '21240': 8, '2130': 9, '21320': 10, '21340': 11, '2140': 12, '21420': 13, '21440': 14, '2150': 15, '21520': 16, '21540': 17, '2160': 18, '21620': 19, '21640': 20, '2170': 21, '21720': 22, '21740': 23, '2180': 24, '21820': 25, '21840': 26, '2190': 27, '21920': 28, '21940': 29, '21100': 30, '211020': 31, '211040': 32, '21110': 33, '211120': 34, '211140': 35, '211220': 37, '211240': 38, '21130': 39, '211320': 40, '211340': 41, '211420': 43, '211440': 44, '21150': 45, '211520': 46, '211540': 47, '21160': 48, '211620': 49, '211640': 50, '21170': 51, '211720': 52, '211740': 53, '21180': 54, '211820': 55, '211840': 56, '21190': 57, '211920': 58, '211940': 59, '21200': 60, '212020': 61, '212040': 62, '21210': 63, '212120': 64, '212140': 65, '212220': 67, '212240': 68, '21230': 69, '212320': 70, '212340': 71}
        const ss = time_index[String(startDay) + String(startHour) + String(startMin)]
        const ee = time_index[String(endDay) + String(endHour) + String(endMin)]
        // console.log(String(startDay) + String(startHour) + String(startMin),String(endDay) + String(endHour) + String(endMin))
    ...
    var histogram = d3.histogram()
        ...
        .thresholds(x.ticks(170)); // then the numbers of bins

    // // And apply twice this function to data to get the bins.
    var bins1 = histogram(data.filter(function (d) {
        return d.type === "remain"
    }));
...
// console.log(m)
                ...
                if (!contextWordSelect.includes(m.name)) {
                    if (contextWordSelect.length >max_select) {
