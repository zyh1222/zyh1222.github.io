...
readTextFile("../../data/data.json", function (text6) {
  ...
  for (let i = 0; i < rawdata.length; i++) {
    var d = rawdata[i];
    if ((d["created_int"] > 1359936000000) && (d["created_int"] < 1391472000000)) {
      rawdata1.push(d)
    }
    if ((d["created_int"] > 1391472000000) && (d["created_int"] < 1423008000000)) {
      rawdata2.push(d)
    }
    if ((d["created_int"] > 1423008000000)){
      rawdata3.push(d)
    }
  }
    })
  ...
  function search (searchquery, options, fromPaging) {
    ...
    if (q) {
      ...
      var doneFunc = function (err, data_u) {
        ...
        const data =  {
          'rows':[],
          'fields':['created_at','text','publisher'],
          'counts':[1]
        }
        ...
        for (let i = 0; i < currRawdata.length; i++) {
          ...
          for (let j = 0; j < searchqueryArray.length; j++) {
            if (!currRawdata[i].text.includes(searchqueryArray[j])){
              found = false
              break
            }
          }
